...
1) the distance between :math:`p_e` and :math:`p_r` is less then `range` ~ 23 cm,
...
The light intensities of all emitters are summed up and stimulate a noisy response :math:`r` :

.. math::

  r ~ \frac{m}{\frac{1}{\sum_e \frac{c-x_0^2}{(|p_r - p_e|-x_0)^2}} + 1}

where :math:`r`~4200 is the maximal value, :math:`x_0`~0.02cm is a small offset and `c`~274 cm:math:`^2` is a bias

We set to 0 all responses not large enough (i.e., lower than the response caused by an emitter at distance :math:`range`).
