...
const LED battery = LED(vec2(0.2773, 0.2373), vec2(0.0244, 0.0391));
...
void main(void) {
  ...
  if (contained_in_led(battery, p)) {
    vec4 led_color = compute_color(led0_color, vec4(1, 1, 0, 1));
    color = mix(color, led_color.xyz, led_color.a);
  } else if (contained_in_led(top, p)) {
